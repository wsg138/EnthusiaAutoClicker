...
public final class AutoclickerRuntime {
    ...
    public void tick(Minecraft client) {
        ...
        while (settingsKey.consumeClick()) {
            if (client.screen == null) {
                client.setScreen(new AutoclickerSettingsScreen(config, null)); // NOPMD - fresh screen per key-open event
            }
        }
    ...
    private static void restorePhysicalState(Minecraft client, KeyMapping mapping) {
        ...
        boolean physicallyDown = switch (key.getType()) {
            ...
            case KEYSYM -> InputConstants.isKeyDown(client.getWindow().getWindow(), key.getValue());
            case SCANCODE -> false;
        };
    ...
    private boolean shouldEatOffhand(Minecraft client, long nowMillis) {
        ...
        boolean offhandFood = isFood(offhand);
        if (!offhandFood) {
            return false;
        }
        ...
        FoodProperties food = offhand.getItem().getFoodProperties();
        boolean readyToEat = client.player.getFoodData().getFoodLevel() <= config.foodLevelThreshold()
            && client.player.canEat(food != null && food.canAlwaysEat())
            && !client.player.getCooldowns().isOnCooldown(offhand.getItem());
        if (!readyToEat) {
            return false;
        }
