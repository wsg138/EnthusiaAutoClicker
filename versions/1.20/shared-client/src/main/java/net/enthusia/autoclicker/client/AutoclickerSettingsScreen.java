...
public final class AutoclickerSettingsScreen extends Screen {
    ...
    public AutoclickerSettingsScreen(AutoclickerConfig config, Screen parent) {
    ...
    public void render(GuiGraphics graphics, int mouseX, int mouseY, float partialTick) {
        ...
        for (ConfigRow row : rows) {
            ...
            if (row.contains(mouseX, mouseY, width)) {
                graphics.renderTooltip(font, row.tooltip(), mouseX, mouseY);
            }
        }
    ...
    private void addTabs() {
        addRenderableWidget(Button.builder(tabLabel(Page.CLICKER), button -> switchPage(Page.CLICKER))
            .bounds(width / 2 - TAB_WIDTH - 2, 31, TAB_WIDTH, 22).build());
        addRenderableWidget(Button.builder(tabLabel(Page.EXTRAS), button -> switchPage(Page.EXTRAS))
            .bounds(width / 2 + 2, 31, TAB_WIDTH, 22).build());
    }
    ...
    private static void setToggleMessage(Button button, boolean enabled) {
        if (button != null) {
